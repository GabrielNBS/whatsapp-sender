    ...
    <div className="space-y-6 max-w-4xl mx-auto">
      ...
      <div className="grid gap-6 md:grid-cols-3">
          ...
          <Card>
            ...
            <CardContent className="space-y-4">
               <div className="space-y-2">
                  ...
                  <Select value={selectedGroupId} onChange={(e) => setSelectedGroupId(e.target.value)} disabled={isSending}>
               ...
               <div className="space-y-2">
                  ...
                  {selectedFile && <p className="text-xs text-green-600">Selecionado: {selectedFile.filename}</p>}
               </div>
        ...
        <div className="space-y-6 flex flex-col h-full">
           <Card className="flex flex-col h-full border-slate-200 bg-slate-50 shadow-inner">
              <CardHeader className="bg-white border-b flex flex-row items-center justify-between space-y-0 py-3">
                 <CardTitle className="text-sm flex items-center gap-2">
                    <Clock className="w-4 h-4 text-slate-500" />
                    Progresso em Tempo Real
                 </CardTitle>
                 {isSending && (
                    <Badge variant="outline" className="text-[10px] font-normal flex gap-1 items-center bg-slate-50">
              ...
              <CardContent className="space-y-4 pt-6 flex-1 flex flex-col min-h-0">
                 <div className="space-y-2 bg-white p-4 rounded-lg border shadow-sm">
                    ...
                    {statusMessage && (
                       <div className="flex items-center gap-2 text-xs text-blue-600 bg-blue-50 p-2 rounded animate-in fade-in transition-all">
                    ...
                    <AnimatePresence mode="popLayout">
                       {logs.length === 0 && !statusMessage && (
                          <motion.div 
                             initial={{ opacity: 0 }} 
                             animate={{ opacity: 1 }} 
                             className="text-slate-400 italic text-center py-10 text-sm"
                          >
